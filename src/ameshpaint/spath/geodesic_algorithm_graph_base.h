...
public:
	...
	void trace_back2(SurfacePoint& destination,
...
template<class Node>
inline void GeodesicAlgorithmGraphBase<Node>::trace_back2(SurfacePoint& destination,   //trace back piecewise-linear path
                             std::vector<SurfacePoint>& path,std::vector<int>& indexVertex)
{
  path.clear();
  indexVertex.clear();
