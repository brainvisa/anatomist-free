...
public:
	...
	virtual void trace_back2(SurfacePoint& destination,
	...
	void geodesic2(SurfacePoint& source,
                SurfacePoint& destination,
...
inline void GeodesicAlgorithmBase::geodesic2(SurfacePoint& source,
                      SurfacePoint& destination,
                      std::vector<SurfacePoint>& path,std::vector<int>& indexVertex) //lazy people can find geodesic path with one function call
{
  ...
  trace_back2(destination, path,indexVertex);
}
